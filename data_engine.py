...
def load_dividend_actual(conn, portfolio_df: pd.DataFrame = None) -> pd.DataFrame:
    try:
        ...
        # 수량 컬럼도 숫자 변환 (시트에 직접 입력된 수량 사용)
        if "수량" in df.columns:
            df["수량"] = pd.to_numeric(
                df["수량"].astype(str).str.replace(",", "").str.replace("-", ""),
                errors="coerce"
            ).fillna(0)

        has_sheet_qty = "수량" in df.columns

        def _get_shares(row) -> float:
            # 1순위: 배당실적 시트의 수량 직접 입력값 (수령 당시 실제 수량)
            if has_sheet_qty:
                sheet_qty = float(row.get("수량", 0) or 0)
                if sheet_qty > 0:
                    return sheet_qty
            # 2순위: portfolio_df(종목현황) 폴백 (시트 미입력 시)
        ...
        if "세후금액" not in df.columns:
            df["세후금액"] = 0.0
        # 시트 세후금액 복원 (수량 재계산과 무관하게 유지)
        if not _sheet_net.empty:
            df["세후금액"] = _sheet_net.values
        # 세후금액이 0인 행만 자동 계산
        mask_zero = df["세후금액"] == 0
        df.loc[mask_zero, "세후금액"] = df.loc[mask_zero, "세전금액"] * (1 - DIVIDEND_TAX_RATE)
...
def calc_avg_cost(trades_df: pd.DataFrame,
    ...
    portfolio_df: 종목현황 시트 — 매수 기록 없이 매도만 있는 경우 매입단가 조회용.
    """
    if trades_df.empty:
        return pd.DataFrame(
    ...
    for (acc, nm), grp in groups:
        ...
        for _, row in grp.iterrows():
            ...
            if 구분 == "매수":
                ...
                # 거래내역에 매수 없는 매도 → 종목현황에서 매입단가 조회
                # 메모에 숫자 입력 시 매입단가로 사용 (예: 메모='124000')
                if qty_hold == 0:
                    memo = str(row.get("메모", "") or "").strip()
                    if memo:
                        try:
                            mp = float(memo.replace(",", ""))
                            if mp > 0:
                                qty_hold = q; cost_total = q * mp
                        except ValueError:
                            pass
...
def merge_trades_to_portfolio(portfolio_df: pd.DataFrame,
    ...
    for _, row in avg_df.iterrows():
        ...
        # 중복 방지: 거래내역 수량 ≤ 시트 수량이면 이미 반영된 상태
        # → 수량 유지, 평균단가만 보정 (예: SK하이닉스 시트1주+거래1주 → 1주 유지)
