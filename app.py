...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v15.2", layout="wide")
...
# 데이터 가공
with st.spinner('실시간 시세 및 인터랙티브 엔진 가동 중...'):
    for c in ['수량', '매입단가']:
...
with tabs[0]:
    ...
    if not history_df.empty:
        ...
        fig.update_layout(
            yaxis=dict(title="상대 수익률 (100 기준)", range=[50, 150], gridcolor="rgba(255,255,255,0.05)"), # 🎯 축 범위 고정
            hovermode="x unified",
    ...
    st.info(f"**📅 {now_kst.strftime('%Y-%m-%d %H:%M')} 시장 분석:** 현재 시장의 변동성 속에서도 고배당주 중심의 포트폴리오는 강력한 방어력을 보여줍니다. 고정된 시야(50~150) 내에서 자산의 장기 우상향 흐름을 관찰하시기 바랍니다.")
...
def render_account_tab(acc_name, tab_obj):
    with tab_obj:
        ...
        st.success(f"현재 {acc_name} 계좌는 시장의 흐름에 유연하게 대응하고 있습니다. 안정적인 배당과 성장이 조화로운 구성입니다.")
