...
st.markdown("""
    ...
    /* 🎯 딥다이브 카드 (v30.9 레이아웃에 맞춰 정교화) */
    .insight-card { background: rgba(135,206,235,0.04); padding: 22px; border-radius: 12px; border: 1px solid rgba(135,206,235,0.25); margin-bottom: 15px; }
...
def get_stock_intelligence(name):
    ...
    try:
        ...
        if summary_div: desc = summary_div.text.strip().split(".")[0] + "."
        elif soup.find("div", {"class": "summary_info"}):
            p_tag = soup.find("div", {"class": "summary_info"}).find("p")
        ...
        if aside:
            # 리서치 목표가
            tp_tag = aside.find("em", {"class": "c_rate"})
            if tp_tag: tp = tp_tag.text + "원"
            
            # 테이블 정밀 파싱 (th 텍스트로 값 찾기)
            for tr in aside.find_all("tr"):
                ...
                if th:
                    ...
                    td_em = tr.find("em")
                    if td_em:
                        if "PER" in th_txt and "배" in tr.text: per = td_em.text + "배"
                        elif "PBR" in th_txt and "배" in tr.text: pbr = td_em.text + "배"
                        elif "배당수익률" in th_txt: div = td_em.text + "%"
        
        return {"desc": desc, "tp": tp, "per": per, "pbr": pbr, "div": div}
    except: return None

# --- [v30.9 공시/지수/가격 파싱 원형] ---
...
with tabs[0]:
    ...
    with rep_l:
        st.markdown(f"""<div class='report-box'>
            ...
            <p>2026년 3월 7일 현재, 국내 증시는 <b>KOSPI 5,000선</b> 돌파 이후 역사적 변곡점에 있습니다. 사용자님 지적대로 지수는 초강세장을 유지 중입니다.</p>
            <div class='market-tag'>KOSPI / KOSDAQ 분석</div>
            <p>삼성전자와 SK하이닉스가 지수 성장을 주도하며, 코스닥은 1,000선을 상회하는 강력한 로봇/AI 순환매가 전개되고 있습니다.</p>
        </div>""", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"""<div class='report-box'>
            <h4 style='color: #FF4B4B;'>🌍 글로벌 시장 및 매크로 분석</h4>
            <div class='market-tag'>환율 및 미 중간선거 변수</div>
            <p>환율은 <b>1,400원 중후반대</b>를 유지하며 수출 실적에 우호적인 환경이며, 하반기 미국 중간선거에 대비한 섹터 전략이 필수적입니다.</p>
        </div>""", unsafe_allow_html=True)
    ...
    sectors = {
        "반도체 / IT": {"주도주": "삼성전자, SK하이닉스", "전망": "AI 수요 폭발 수혜."},
        "전력 / ESS": {"주도주": "LS ELECTRIC, 일진전기", "전망": "북미 인프라 교체 주기."},
        "배터리 / 에너지": {"주도주": "LG에너지솔루션, 에코프로", "전망": "전고체 기술 기대감."},
...
# [계좌별 상세 분석 탭: v30.9 원형 유지 + 딥다이브 보강]
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with g1:
            ...
            # 🎯 [딥다이브 카드 정밀 수정] 태그 매칭 방식으로 데이터 보강
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v31.5 파이널 가디언")
