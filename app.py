...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v27.5", layout="wide")
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v27.5</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with rep_l:
        st.markdown(f"<div style='background-color: rgba(135,206,235,0.05); padding: 15px; border-radius: 10px; height: 300px;'><h4 style='color: #87CEEB;'>📋 통합 포트폴리오 성과</h4><ul style='font-size: 0.95em;'><li><b>지수 대비 성과:</b> KOSPI 대비 <b>{total_daily_rate - float(m_info['KOSPI']['rate'].replace('%','')) :+.2f}%p</b> {'초과 수익' if total_daily_rate > float(m_info['KOSPI']['rate'].replace('%','')) else '하회'} 중</li><li><b>수익 현황:</b> 현재 총 <b>{total_profit:,.0f}원</b>의 누적 손익을 기록 중입니다.</li></ul></div>", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"<div style='background-color: rgba(255,75,75,0.05); padding: 15px; border-radius: 10px; height: 300px;'><h4 style='color: #FF4B4B;'>🌍 시장 동향 리포트</h4><ul style='font-size: 0.95em;'><li><b>섹터 동향:</b> 에너지 및 방산 섹터의 견고한 흐름이 지수 하방을 지지하고 있습니다.</li></ul></div>", unsafe_allow_html=True)
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with ar_l:
            st.markdown(f"<div style='background-color: rgba(135,206,235,0.05); padding: 15px; border-radius: 10px; height: 150px;'><h4 style='color: #87CEEB;'>📋 성과 분석</h4><p>현재 계좌 내 누적 손익은 <b>{acc_profit:,.0f}원</b>입니다.</p></div>", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"<div style='background-color: rgba(255,75,75,0.05); padding: 15px; border-radius: 10px; height: 150px;'><h4 style='color: #FF4B4B;'>🌍 분석 의견</h4><p>시장 대비 변동성을 낮추며 안정적인 수익 구조를 형성하고 있습니다.</p></div>", unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v27.5 수익성 지표 가시화 완료")
