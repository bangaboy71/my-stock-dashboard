...
st.set_page_config(page_title="가족 자산 성장 관제탑 v36.2", layout="wide")
...
st.markdown("""
    ...
    .insight-right { flex: 1; background: rgba(255,215,0,0.03); padding: 18px; border-radius: 8px; border-left: 4px solid #FFD700; }
    ...
    .implication-title { color: #FFD700; font-weight: bold; font-size: 1rem; margin-bottom: 12px; display: flex; align-items: center; gap: 8px; }
    .implication-item { font-size: 0.9rem; color: rgba(255,255,255,0.95); margin-bottom: 10px; line-height: 1.6; }
...
# --- [2. 연구 데이터베이스 통합 (CSV 내용 반영)] ---
STOCK_CODES = {"삼성전자": "005930", "KT&G": "033780", "LG에너지솔루션": "373220", "현대글로비스": "086280", "현대차2우B": "005387", "KODEX200타겟위클리커버드콜": "498400", "에스티팜": "237690", "테스": "095610", "일진전기": "103590", "SK스퀘어": "402340"}

RESEARCH_DATA = {
    "삼성전자": {
        "desc": "반도체 업황 회복 및 HBM3E 공급 본격화에 따른 역사적 이익 가시성 확보.",
    ...
    "KT&G": {
        "desc": "3개년 주주환원 정책 기반의 밸류업 선도주.",
        "metrics": [("영업이익률", "20.5%", "20.8%"), ("ROE", "10.5%", "15.0%"), ("PBR", "1.25배", "1.40배"), ("정규 DPS", "6,000원", "6,400~6,600원")],
        "implications": ["해외 궐련 수출 확대 및 NGP(전자담배) 글로벌 점유율 상승", "2027년까지 발행주식 20% 소각을 통한 주당 가치 제고", "카자흐스탄 신공장 가동을 통한 유라시아 시장 지배력 강화"]
    },
    "현대차2우B": {
        "desc": "은퇴 포트폴리오의 핵심 캐시카우 및 고배당 우선주.",
        "metrics": [("영업이익률", "6.2%", "7.0%"), ("ROE", "13.0%", "15.5%"), ("PER", "10.5배", "8.2배"), ("시가배당률", "5.7%", "6.4%")],
        "implications": ["고부가가치 차종(SUV, 제네시스) 믹스 개선을 통한 수익성 가이드라인 충족", "본주 대비 높은 할인율로 저평가 매력 및 배당수익률 극대화", "배당 성향 25% 유지 및 분기 배당을 통한 안정적 현금 흐름"]
    },
    "KODEX200타겟위클리커버드콜": {
        "desc": "매주 분배 재원을 확보하는 타겟 프리미엄 전략 ETF.",
        "metrics": [("옵션 프리미엄", "연 15%", "연 15%"), ("월 분배금", "100~120원", "110~130원"), ("연환산 수익률", "12~15%", "15% 이상"), ("시가분배율", "연 12.5%", "연 15.0%")],
        "implications": ["지수 박스권 정체 시 콜옵션 매도 수익을 통한 인컴 창출 극대화", "매월 안정적인 현금 흐름을 필요로 하는 은퇴 생활비 마련의 핵심", "상승장 일부 참여 및 하락장 방어력을 갖춘 하이브리드 전략"]
    }
    # (추가 종목 생략, 동일 로직 적용)
}
...
if st.sidebar.button("💾 오늘의 결과 저장/덮어쓰기"):
    ...
    st.sidebar.success(f"✅ 결과 저장 완료!"); st.rerun()
...
# --- [UI 메인 구성] ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v36.2</h1>", unsafe_allow_html=True)
tabs = st.tabs(["📊 총괄 현황", "💰 서은투자", "📈 서희투자", "🙏 큰스님투자"])

# [Tab 0] 총괄 현황 (블랙아웃 방지 및 원형 복구)
with tabs[0]:
    ...
    with rep_l: st.markdown("<div class='report-box'><h4 style='color:#87CEEB;'>🇰🇷 국내 시장 분석</h4><p>국내 증시는 강력한 이익 체력을 바탕으로 선진국형 랠리를 이어가고 있습니다.</p></div>", unsafe_allow_html=True)
    with rep_r: st.markdown("<div class='report-box'><h4 style='color:#FF4B4B;'>🌍 글로벌 매크로 분석</h4><p>나스닥 AI 랠리와 고환율 환경이 대형 수출주에 우호적인 환경을 제공합니다.</p></div>", unsafe_allow_html=True)

# [계좌별 탭] (기본 기능 복구 및 시사점 우측 조정)
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        # 🎯 [레이아웃 조정] 딥다이브 카드 내 시사점을 우측으로 배치
        ...
        with ar_l: st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color:#87CEEB;'>📋 계좌 총평</h4><p>{acc_name} 포트폴리오는 견조한 흐름을 유지 중입니다.</p></div>", unsafe_allow_html=True)
        with ar_r: st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color:#FF4B4B;'>🌍 업황 대응 전략</h4><p>연구 자료의 시사점을 바탕으로 리밸런싱을 검토합니다.</p></div>", unsafe_allow_html=True)
        ...
        if acc_news:
            ...
            st.markdown(f"<div class='acc-flash-container'><div style='font-weight: bold; color: #FFD700; margin-bottom: 10px;'>🔔 보유종목 최신 뉴스 및 공시</div>{news_html}</div>", unsafe_allow_html=True)

render_account_tab("서은투자", tabs[1], "서은수익률")
render_account_tab("서희투자", tabs[2], "서희수익률")
render_account_tab("큰스님투자", tabs[3], "큰스님수익률")

st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v36.2 리서치 커맨드 센터")
