...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v24.6", layout="wide")
...
def record_performance(overwrite=False):
    ...
    new_row = {
        ...
        "서은_삼성전자수익률": get_stock_yield('서은투자', '삼성전자'),
        "서희_삼성전자수익률": get_stock_yield('서희투자', '삼성전자')
    }
    try:
    ...
    with tab_obj:
        ...
        with col_c1:
            if not history_df.empty and history_col in history_df.columns:
                ...
                # 🎯 보정: 계좌명에서 '투자'를 떼고 시트의 '서은_삼성전자수익률' 형식과 매칭
                short_acc_name = acc_name.replace("투자", "")
                history_stock_col = f"{short_acc_name}_{compare_stock}수익률"
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v24.6 열 매칭 보정 완료")
