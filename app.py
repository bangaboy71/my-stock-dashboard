...
# --- [CSS: v30.9 스타일 완벽 유지 및 하이퍼링크 효과] ---
st.markdown("""
    <style>
...
def record_performance():
    ...
    st.sidebar.success("✅ 결과 저장 완료!"); st.cache_data.clear(); st.rerun()
...
with tabs[0]:
    ...
    with rep_l:
        st.markdown(f"""<div class='report-box'>
            ...
            <p>2026년 3월 7일 현재, KOSPI는 <b>5,000선 중반</b>에 안착하며 강력한 펀더멘털 기반의 우상향 기조를 확고히 하고 있습니다. 사용자님께서 지적하신 대로 지수는 역사적 고점 부근에서 강력한 수급의 지지를 받고 있습니다.</p>
            <div class='market-tag'>KOSPI / KOSDAQ 상세 분석</div>
            <p>삼성전자와 SK하이닉스 등 시총 상위주가 지수 성장을 주도하고 있습니다. 코스닥 역시 1,000선을 안정적으로 상회하며 바이오와 로봇 섹터의 강력한 모멘텀이 지수를 강력히 지지하고 있습니다.</p>
        </div>""", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"""<div class='report-box'>
            <h4 style='color: #FF4B4B;'>🌍 글로벌 시장 및 매크로 분석</h4>
            <div class='market-tag'>미국 증시 및 환율 상황</div>
            <p>나스닥은 기술주 중심의 강력한 랠리 속에 AI 실질 실적 검증 단계에 진입했습니다. 현재 환율은 <b>1,400원 중후반대(1,450~1,480원)</b>를 기록하며 고환율 장기화 국면에 있습니다.</p>
            <div class='market-tag'>2026 미 중간선거 변수</div>
            <p>하반기 미국 중간선거를 앞둔 보호무역주의 강화 움직임은 수출 비중이 높은 우리 가족 포트폴리오의 대형주들에게 기회와 리스크를 동시에 제공하고 있습니다.</p>
        </div>""", unsafe_allow_html=True)
    ...
    sectors = {
        "반도체 / IT": {"주도주": "삼성전자, SK하이닉스", "시황": "AI 칩 수요 폭발 지속.", "전망": "하반기 실적 퀀텀 점프 기대."},
        "전력 / ESS": {"주도주": "LS ELECTRIC, 일진전기", "시황": "미국 인프라 교체 수혜.", "전망": "장기 슈퍼 사이클 진입."},
        "배터리 / 에너지": {"주도주": "LG에너지솔루션, 에코프로비엠", "시황": "전고체 상용화 로드맵 가속.", "전망": "기술 초격차 전략 유지."},
        "바이오 / 헬스케어": {"주도주": "삼성바이오로직스, 알테오젠", "시황": "기술 수출 모멘텀 강화.", "전망": "글로벌 시장 재평가 가속."},
        "모빌리티 / 로봇": {"주도주": "현대차, 두산로보틱스", "시황": "휴머노이드 상용화 기대감.", "전망": "수출 확대 및 시장 개화."},
        "소비재 / 뷰티": {"주도주": "아모레퍼시픽, 브이티", "시황": "K-뷰티 북미 장악력 폭증.", "전망": "실적 랠리 지속 전망."}
    }
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with ar_l: st.markdown(f"<div class='report-box' style='height:220px;'><h4 style='color: #87CEEB;'>📋 계좌 총평</h4><p>현재 계좌는 시장 주도주를 바탕으로 지수 수익률을 상회하고 있습니다.</p></div>", unsafe_allow_html=True)
        with ar_r: st.markdown(f"<div class='report-box' style='height:220px;'><h4 style='color: #FF4B4B;'>🌍 업황 대응</h4><p>고환율 수혜를 입는 수출 우량주 비중을 유지하며 변동성에 대응 중입니다.</p></div>", unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v30.9 마스터피스 리스토어")
