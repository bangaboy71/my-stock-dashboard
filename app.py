...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v27.4", layout="wide")
...
if not history_df.empty and any(history_df['Date'] == today_date):
    if st.sidebar.button("♻️ 오늘 데이터 덮어쓰기"): pass # 성과 저장 로직 생략
else:
    if st.sidebar.button("💾 오늘의 결과 저장하기"): pass # 성과 저장 로직 생략

# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v27.4</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with rep_l:
        st.markdown(f"<div style='background-color: rgba(135,206,235,0.05); padding: 15px; border-radius: 10px; height: 300px;'><h4 style='color: #87CEEB;'>📋 통합 포트폴리오 성과</h4><ul style='font-size: 0.95em;'><li><b>지수 대비 성과:</b> KOSPI 대비 <b>{total_daily_rate - float(m_info['KOSPI']['rate'].replace('%','')) :+.2f}%p</b> {'초과 수익' if total_daily_rate > float(m_info['KOSPI']['rate'].replace('%','')) else '하회'} 중</li><li><b>주요 특징:</b> 에너지 및 전력 인프라 섹터 비중이 지수 방어에 기여 중.</li></ul></div>", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"<div style='background-color: rgba(255,75,75,0.05); padding: 15px; border-radius: 10px; height: 300px;'><h4 style='color: #FF4B4B;'>🌍 시장 동향 리포트</h4><ul style='font-size: 0.95em;'><li><b>섹터 동향:</b> 반도체 대형주 중심 외인 매도세 있으나 에너지 섹터 개인 매수 강세.</li><li><b>주요 이슈:</b> 지정학적 리스크에 따른 안전자산 선호 심리 강화.</li></ul></div>", unsafe_allow_html=True)
    ...
    with tab_obj:
        ...
        with g_col1:
            if not history_df.empty and history_col in history_df.columns:
                ...
                if len(stocks) > 1:
                    sel_s = st.selectbox(f"📍 {acc_name} 종목 대조", stocks, key=f"sel_{acc_name}")
                    s_col = next((c for c in history_df.columns if acc_name[:2] in c and sel_s.replace(' ','') in c.replace(' ','')), "")
                    if s_col:
                        fig.add_trace(go.Scatter(x=h_dates, y=history_df[s_col], name=f'{sel_s} 수익률', line=dict(color='#FF4B4B', width=2, dash='dot')))
        ...
        best_s = sub_df.sort_values('전일대비(%)', ascending=False).iloc[0]
        worst_s = sub_df.sort_values('전일대비(%)', ascending=True).iloc[0]
        
        ar_l, ar_r = st.columns(2)
        with ar_l:
            st.markdown(f"<div style='background-color: rgba(135,206,235,0.05); padding: 15px; border-radius: 10px; height: 250px;'><h4 style='color: #87CEEB;'>📋 계좌 내 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>베스트 종목:</b> {best_s['종목명']} ({best_s['전일대비(%)']:+.2f}%)</li><li><b>워스트 종목:</b> {worst_s['종목명']} ({worst_s['전일대비(%)']:+.2f}%)</li><li><b>상태:</b> KOSPI 대비 <b>{a_daily_rate - float(get_market_status()['KOSPI']['rate'].replace('%','')):+.2f}%p</b> 변동 중</li></ul></div>", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"<div style='background-color: rgba(255,75,75,0.05); padding: 15px; border-radius: 10px; height: 250px;'><h4 style='color: #FF4B4B;'>🌍 보유 종목 섹터 분석</h4><ul style='font-size: 0.95em;'><li><b>주요 섹터 동향:</b> {best_s['종목명']} 관련 섹터의 수급 흐름이 계좌 수익률을 견인 중.</li><li><b>특이사항:</b> {acc_name} 계좌는 지수 연동성보다 개별 종목 재료에 민감하게 반응하고 있습니다.</li></ul></div>", unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v27.4 계좌별 리포트 최적화 및 레이아웃 복구")
