...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v31.1", layout="wide")

# --- [CSS: v30.8 스타일 완벽 복구 및 시인성 강화] ---
st.markdown("""
    <style>
    ...
    .insight-title { color: #87CEEB; font-weight: bold; font-size: 1.1rem; margin-bottom: 10px; border-bottom: 1px solid rgba(135,206,235,0.2); padding-bottom: 5px; }
    ...
    try:
        ...
        if aside:
            ...
            for tr in aside.find_all("tr"):
                if "PER" in tr.text and "배" in tr.text: per = tr.find("em").text + "배"
                if "PBR" in tr.text and "배" in tr.text: pbr = tr.find("em").text + "배"
                if "배당수익률" in tr.text and "%" in tr.text: div = tr.find("em").text + "%"
        
        return {"desc": desc, "tp": tp, "per": per, "pbr": pbr, "div": div}
    except: return None

# --- [공시 및 지수 파싱: v30.9 표준 고수] ---
...
if not full_df.empty:
    ...
    full_df['매입금액'] = (full_df['수량'] * full_df['매입단가']).astype(int)
    full_df['평가금액'] = (full_df['수량'] * full_df['현재가']).astype(int)
    full_df['전일평가금액'] = (full_df['수량'] * full_df['전일종가']).astype(int)
    full_df['주가변동'] = (full_df['현재가'] - full_df['매입단가']).astype(int)
    full_df['손익'] = (full_df['평가금액'] - full_df['매입금액']).astype(int)
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v31.1</h1>", unsafe_allow_html=True)
tabs = st.tabs(["📊 총괄 현황", "💰 서은투자", "📈 서희투자", "🙏 큰스님투자"])

# [Tab 0] 총괄 현황: v30.9 레이아웃 복구
with tabs[0]:
    ...
    with rep_l: st.markdown(f"<div class='report-box'><h4 style='color:#87CEEB;'>🇰🇷 국내 시장 분석</h4><div class='market-tag'>데일리 총평</div><p>2026년 3월 7일 현재, KOSPI는 <b>5,000선 시대</b>를 활짝 열었습니다. 사용자님의 원칙대로 실시간 지수를 동기화하여 역사적 고점 국면임을 확인합니다.</p></div>", unsafe_allow_html=True)
    with rep_r: st.markdown(f"<div class='report-box'><h4 style='color:#FF4B4B;'>🌍 매크로 전략</h4><div class='market-tag'>환율 및 선거</div><p>1,400원 중후반 고환율과 하반기 미국 중간선거 변수가 공존하는 시점입니다. 수출 대형주 중심의 우위를 점검하십시오.</p></div>", unsafe_allow_html=True)

# [계좌별 상세 분석 탭: 정밀 복구 및 딥다이브 보강]
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        sub_df = full_df[full_df['계좌명'] == acc_name].copy()
        if sub_df.empty: return
        
        # 🎯 [오류 수정] 누적 수익률 하단에 전일 대비 변동률(delta) 병기 복구
        ...
        with g1:
            ...
            sel_stk = st.selectbox(f"📍 {acc_name} 종목 인텔리전스", stk_list, key=f"sel_{acc_name}")
            
            # 🎯 [오류 수정] 딥다이브 카드 정보 로딩 보강
            intel = get_stock_intelligence(sel_stk)
            if intel:
                st.markdown(f"""
                <div class='insight-card'>
                    <div class='insight-title'>🔍 {sel_stk} 기업 분석</div>
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v31.1 정밀 관제 버전")
