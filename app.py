...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v29.4", layout="wide")

# --- [CSS: 메트릭, 리포트, 섹터 박스 스타일] ---
st.markdown("""
    <style>
...
# --- [🛠️ 헬퍼 함수: 수치 변환 안전화] ---
def safe_float(text):
    """문자열에서 숫자, 소수점, 부호만 추출하여 float로 변환 (ValueError 방지)"""
    try:
...
def get_market_status():
    ...
    try:
        ...
        market["KOSPI"] = {"now": "0", "diff": "0", "rate": "0%"}
        market["KOSDAQ"] = {"now": "0", "diff": "0", "rate": "0%"}
    return market
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v29.4</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    items = [("KOSPI", "KOSPI"), ("KOSDAQ", "KOSDAQ"), ("USD/KRW", "환율"), ("GOLD", "금 현물")]
    ...
    # 🕵️ AI 심층 리포트 (ValueError 방지 보정)
    st.divider()
    st.subheader("🕵️ AI 관제탑 데일리 심층 리포트")
    rep_l, rep_r = st.columns(2)
    with rep_l:
        # 🎯 safe_float 사용하여 ValueError 원천 차단
    ...
    with rep_r:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05);'>
            ...
            <div class='market-tag'>KOSPI 시장</div><p style='font-size: 0.8em;'>변동: {m_status.get('KOSPI',{}).get('rate','-')} | 주도: 전력기기/에너지 | 수급: 외인 매도/기관 매수 | 전망: 상방 압력 지속</p>
            <div class='market-tag'>미국 증시</div><p style='font-size: 0.8em;'>변동: {m_status.get('NASDAQ',{}).get('rate','-')} | 주도: AI 빅테크 | 전망: 강세장 유지</p>
        </div>""", unsafe_allow_html=True)

    # 📊 관심 섹터 동향 분석 (v28.2 5대 요소 유지)
    st.divider()
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v29.4 ValueError 완전 보정 버전")
