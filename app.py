...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v30.8", layout="wide")

# --- [CSS: v30.2 마스터 스타일 및 공시 스타일 추가] ---
st.markdown("""
    <style>
    ...
    try:
        for s in stocks:
            ...
            if news_sec:
                ...
                if top_news:
                    title = top_news.find("a").text.strip()
                    news_list.append({"name": s, "title": title})
    except: pass
    return news_list
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v30.8</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    sectors = {
        "반도체 / IT": {"주도주": "삼성전자, SK하이닉스, 한미반도체", "시황": "AI 칩 수요 폭발 및 HBM3E 공급 부족 지속.", "수급": "외인/기관 동반 순매수세 집중.", "뉴스": "차세대 패키징 공정 수율 개선 공시.", "전망": "하반기 실적 퀀텀 점프 예상."},
        "전력 / ESS": {"주도주": "LS ELECTRIC, 일진전기, 현대일렉트릭", "시황": "북미 노후 전력망 교체 및 AI 데이터센터 전력난 수혜.", "수급": "수주 잔고 기반 실적주로서 외인 매집 지속.", "뉴스": "변압기 수주 잔고 사상 최대.", "전망": "장기 슈퍼 사이클 진입 가시화."},
        "배터리 / 에너지": {"주도주": "LG에너지솔루션, 에코프로비엠, POSCO홀딩스", "시황": "전기차 캐즘 국면 통과 및 전고체 기술 기대감.", "수급": "개인 매수세 결집 국면이나 외인 숏커버링 포착.", "뉴스": "차세대 폼팩터 샘플 공급.", "전망": "정책 변동성 통과 후 기술력 중심 재편."},
        "바이오 / 헬스케어": {"주도주": "삼성바이오로직스, 알테오젠, HLB", "시황": "금리 인하 기대감과 개별 임상 결과의 하모니.", "수급": "기술 수출 기대감에 따른 스마트 머니 유입.", "뉴스": "글로벌 빅파마와 대규모 라이선스 아웃.", "전망": "시가총액 재평가 가속화 국면."},
        "모빌리티 / 로봇": {"주도주": "현대차, 두산로보틱스, 레인보우로보틱스", "시황": "휴머노이드 상용화 및 고환율에 따른 수출 경쟁력.", "수급": "기관 장기 포트폴리오 내 비중 확대.", "뉴스": "지능형 로봇법 개정안 통과.", "전망": "스마트 팩토리 시장 개화의 주역."},
        "소비재 / 뷰티": {"주도주": "아모레퍼시픽, 브이티, 코스메카코리아", "시황": "K-뷰티의 북미 시장 점유율 폭증.", "수급": "실적 서프라이즈 기반의 외인 지속 매수.", "뉴스": "해외 유통 채널 다변화 및 최대 실적 달성.", "전망": "안정적 영업이익률 확보 지속."}
    }
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with ar_l:
            st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color: #87CEEB;'>📋 계좌 총평</h4><p>현재 {acc_name} 계좌는 {b_s['종목명']}의 주도력에 힘입어 안정적인 흐름을 유지하고 있습니다. 초강세 지수 환경 속에서도 보유 종목의 핵심 기술력과 실적 모멘텀이 변함없이 견조함을 확인 중입니다.</p></div>", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color: #FF4B4B;'>🌍 업황 대응 전략</h4><p>고환율 변동성이 실적에 미치는 긍정적 효과를 극대화하기 위해 수출 대형주 중심의 비중을 유지하며, 미 중간선거 변수에 따른 정책 리스크를 실시간 모니터링 중입니다.</p></div>", unsafe_allow_html=True)

        # 🎯 [신규] 리포트 하단: 각 계좌별 보유종목 실시간 주요 공시 알림
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        if acc_news:
            ...
            <div class='acc-flash-container'>
                <div style='font-weight: bold; color: #FFD700; margin-bottom: 10px;'>🔔 {acc_name} 보유종목 최신 공시/뉴스 플래시</div>
                {" ".join([f"<div class='acc-flash-item'><span class='acc-flash-stock'>[{n['name']}]</span> {n['title']}</div>" for n in acc_news])}
            </div>
            """, unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v30.8 리스토어 앤 플래시 버전")
