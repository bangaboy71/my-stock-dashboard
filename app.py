    ...
    for i, (name, data) in enumerate(sectors.items()):
        with sec_cols[i % 3]:
            ...
            <div class='sector-box'>
                ...
                <p style='font-size: 0.85em; margin-bottom: 5px;'><b>🌡️ 당일 시황:</b> {data['trend']}</p>
                <p style='font-size: 0.85em; margin-bottom: 5px;'><b>👥 수급 정보:</b> {data['supply']}</p>
                <p style='font-size: 0.85em; margin-bottom: 5px;'><b>👑 주도 종목:</b> {data['leader']}</p>
                <p style='font-size: 0.85em; margin-bottom: 5px;'><b>📰 주요 뉴스:</b> {data['news']}</p>
            </div>
            """, unsafe_allow_html=True)

# [계좌별 상세 분석 탭 - v27.8 유지]
