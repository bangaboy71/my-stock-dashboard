...
def check_and_toast_targets(df):
    ...
    if df.empty: return
    if '현재가' not in df.columns or '목표가' not in df.columns: return
    ...
    for _, row in df.iterrows():
        ...
        if curr <= 0 or target <= 0:
            continue

        ratio = curr / target  # 1.0 = 목표가 정확히 도달

        # 이미 알림을 보낸 종목은 건너뜀 (새로고침 전까지 중복 방지)
        if name in alerted:
            continue
        ...
        if ratio >= 1.0:
            ...
            alerted.add(name)
        elif ratio >= 0.95:  # 목표가의 95% 이상 — 근접 경보
            st.toast(
                f"📡 **{name}** 목표가 근접 (95%)  \n"
                f"현재가 {curr:,.0f}원 / 목표가 {target:,.0f}원  \n"
