...
# 1. 설정 및 연결 (v31.6 원형 및 사용자 원칙 사수)
st.set_page_config(page_title="가족 자산 성장 관제탑 v36.7", layout="wide")
...
RESEARCH_DATA = {
    ...
    "현대차2우B": {"desc": "고배당 우선주 및 은퇴 포트폴리오의 캐시카우.", "metrics": [("영업이익률", "6.2%", "7.0%"), ("ROE", "13.0%", "15.5%"), ("시가배당률", "5.7%", "6.4%"), ("정규 DPS", "13,600원", "14.5~15.5천원")], "implications": ["SUV/제네시스 중심 믹스 개선 및 수익 가이드라인 준수", "본주 대비 높은 할인율로 배당수익률 극대화", "자사주 소각 등 적극적 밸류업 정책 수행"]},
...
def color_delta(val):
    if isinstance(val, (int, float)):
        return f'color: {"#FF4B4B" if val > 0 else "#87CEEB" if val < 0 else "white"}'
    return ''
...
# --- [UI 메인 구성] ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v36.7</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    st.divider()
    st.subheader("투자 주체별 성과 요약 (전일 대비 지표 포함)")
    ...
    st.dataframe(sum_acc.style.map(color_delta, subset=['손익', '누적수익률', '전일대비손익', '전일대비변동률']).format({
        '매입금액':'{:,.0f}원', '평가금액':'{:,.0f}원', '손익':'{:+,.0f}원', '누적수익률':'{:+.2f}%', '전일대비손익':'{:+,.0f}원', '전일대비변동률':'{:+.2f}%'
    ...
    st.divider()
    st.subheader("🕵️ AI 관제탑 데일리 심층 리포트 및 지수")
    ...
    with rep_l: st.markdown("<div class='report-box'><h4 style='color:#87CEEB;'>🇰🇷 국내 시장 분석</h4><p>국내 증시는 강력한 수출 실적을 바탕으로 코스피 5,000선 시대를 열고 있습니다. 특히 반도체와 모빌리티 섹터의 이익 기여도가 높아지며 지수의 하방 경직성을 확보하고 있습니다.</p></div>", unsafe_allow_html=True)
    with rep_r: st.markdown("<div class='report-box'><h4 style='color:#FF4B4B;'>🌍 글로벌 매크로 분석</h4><p>미 연준의 금리 기조와 글로벌 AI 인프라 투자 사이클이 국내 대형 IT주에 우호적으로 작용하고 있습니다. 환율 변동성은 수출 기업의 영업이익률을 지지하는 요소로 작용 중입니다.</p></div>", unsafe_allow_html=True)

    st.divider()
    st.subheader("📊 관심 섹터별 인텔리전스")
    s_cols = st.columns(3)
    sectors = {"반도체 / IT": "HBM 수요 폭발 및 AI 서버 증설 수혜.", "전력 / ESS": "북미 인프라 교체 및 데이터센터 증설.", "배터리 / 에너지": "전고체 및 차세대 배터리 점유율 확대.", "바이오 / CDMO": "RNA 치료제 및 위탁생산 모멘텀.", "모빌리티 / 전장": "현대차 그룹 중심 밸류업 및 하이브리드 강세.", "소비재 / 뷰티": "K-뷰티의 글로벌 시장 점유율 급증."}
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with ar_l: st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color:#87CEEB;'>📋 계좌 총평</h4><p>{acc_name} 포트폴리오는 연구 자료의 가이드라인에 따라 견조하게 관리되고 있습니다. 장기적인 목표 가치(Target) 도달까지 변동성을 방어하며 보유 비중을 조절 중입니다.</p></div>", unsafe_allow_html=True)
        with ar_r: st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color:#FF4B4B;'>🌍 업황 대응 전략</h4><p>글로벌 금리 및 정책 환경 변화에 따라 성장주와 가치주의 비중을 최적화합니다. 특히 현금 흐름이 확보된 주주 친화 기업 위주로 리스크를 관리합니다.</p></div>", unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v36.7 가디언 컴플리트 싱크")
