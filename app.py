...
# 1. 설정 및 UI 스타일 (v36.5 원형 기반 복구)
st.set_page_config(page_title="가족 자산 성장 관제탑 v36.27", layout="wide")

st.markdown("""
    <style>
    [data-testid="stMetricValue"] { font-size: 1.8rem !important; font-weight: bold !important; }
    .report-box { padding: 25px; border-radius: 12px; height: 320px; overflow-y: auto; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.15); background-color: rgba(255,255,255,0.02); line-height: 1.8; }
...
# --- [4. 유연한 열 매칭 함수 (KeyError 방지)] ---
def get_col_safe(df, key):
    # 공백 제거 후 비교하여 유사한 열 이름을 찾아 반환
...
with st.sidebar:
    ...
    if st.button("오늘의 결과 확정 및 저장"):
        ...
        for acc in ['서은투자', '서희투자', '큰스님투자']:
            col_name = get_col_safe(history_df, f"{acc}수익률")
            if col_name: new_row[col_name] = acc_sum.get(acc, 0)
        
        for _, row in full_df.iterrows():
            target_key = f"{row['계좌명']}_{row['종목명']}수익률"
            col_name = get_col_safe(history_df, target_key)
            if col_name: new_row[col_name] = row['누적수익률']
    ...
    if not history_df.empty:
        ...
        if st.button("해당 날짜 데이터 삭제"):
            ...
            st.warning(f"🗑️ {clean_date} 삭제됨."); st.rerun()

# --- [6. UI 메인 구성] ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v36.27</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    st.dataframe(sum_acc.style.apply(lambda row: ['' if i != 2 else ('color: #FF4B4B' if row[2]>row[1] else 'color: #87CEEB') for i, v in enumerate(row)], axis=1).format({'매입금액':'{:,.0f}원', '평가금액':'{:,.0f}원', '손익':'{:+,.0f}원', '전일대비변동률':'{:+.2f}%', '누적수익률':'{:+.2f}%'}), use_container_width=True, hide_index=True)
    ...
    if not history_df.empty:
        ...
        for acc in ['서은투자수익률', '서희투자수익률', '큰스님투자수익률']:
            ...
            if col:
                fig.add_trace(go.Scatter(x=h_dates, y=history_df[col], mode='lines+markers', name=col, line=dict(width=3)))
    ...
    s_cols = st.columns(3)
    sectors = {"반도체 / IT": "HBM 수요 폭발 및 AI 서버 증설.", "전력 / ESS": "북미 인프라 교체 및 데이터센터.", "배터리 / 에너지": "전고체 기술 점유율 확대."}
    ...
    with tab_obj:
        ...
        with g_left:
            if not history_df.empty:
                ...
                # 🎯 KeyError 방지: 열 존재 확인 후 드로잉
                acc_col = get_col_safe(history_df, history_col_key)
                if acc_col:
                    fig_acc.add_trace(go.Scatter(x=h_dt, y=history_df[acc_col], mode='lines+markers', name=f'{acc_name} 실제수익률', line=dict(width=4)))
                
                s_key = f"{acc_name}_{sel}수익률"
                s_c = get_col_safe(history_df, s_key)
                if s_c and s_c != acc_col:
                    fig_acc.add_trace(go.Scatter(x=h_dt, y=history_df[s_c], mode='lines', name=f'{sel} 실제수익률', line=dict(width=2, dash='dot')))
        ...
        with r_l: st.markdown(f"<div class='report-box'><h4>📋 {acc_name} 총평</h4><p>Target 달성 보유 지속.</p></div>", unsafe_allow_html=True)
        with r_r: st.markdown("<div class='report-box'><h4>🌍 업황 전략</h4><p>시장 변동성 대응 모니터링.</p></div>", unsafe_allow_html=True)

render_account_tab("서은투자", tabs[1], "서은투자수익률")
render_account_tab("서희투자", tabs[2], "서희투자수익률")
render_account_tab("큰스님투자", tabs[3], "큰스님투자수익률")

st.caption(f"v36.27 가디언 프리시전 세이프가드 | {now_kst.strftime('%Y-%m-%d %H:%M:%S')}")
