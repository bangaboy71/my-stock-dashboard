...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v28.3", layout="wide")
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v28.3</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with rep_l:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(135,206,235,0.05); border: 1px solid rgba(135,206,235,0.2);'><h4 style='color: #87CEEB;'>📋 통합 포트폴리오 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>지수 대비 성과:</b> KOSPI 대비 <b>{total_daily_rate - float(m_info['KOSPI']['rate'].replace('%','')) :+.2f}%p</b> {'초과 수익' if total_daily_rate > float(m_info['KOSPI']['rate'].replace('%','')) else '하회'} 중</li></ul></div>""", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05); border: 1px solid rgba(255,75,75,0.2);'><h4 style='color: #FF4B4B;'>🌍 시장 동향 및 수급 리포트</h4><ul style='font-size: 0.95em;'><li><b>섹터 동향:</b> 에너지 및 방산 섹터의 견조한 흐름이 지수 하방을 지지하고 있습니다.</li></ul></div>""", unsafe_allow_html=True)
    ...
    sectors = {
        "반도체": {"시황": "HBM 수요 견조 및 공급망 다변화", "수급": "외인 매도 vs 기관 매수", "주도": "삼성전자, SK하이닉스", "뉴스": "AI 반도체 시장 확대에 따른 후공정 장비주 강세", "전망": "데이터센터 투자 확대에 따른 실적 우상향 지속 예상"},
    ...
    for i, (name, d) in enumerate(sectors.items()):
        with sec_cols[i % 3]:
            st.markdown(f"""<div class='sector-box'><div class='sector-title'>{name}</div><p style='font-size: 0.85em; margin-bottom: 4px;'><b>🌡️ 시황:</b> {d['시황']}</p><p style='font-size: 0.85em; margin-bottom: 4px;'><b>👥 수급:</b> {d['수급']}</p><p style='font-size: 0.85em; margin-bottom: 4px;'><b>👑 주도:</b> {d['주도']}</p><p style='font-size: 0.85em; margin-bottom: 4px;'><b>📰 뉴스:</b> {d['뉴스']}</p><p style='font-size: 0.85em; margin-bottom: 4px; color: #FFD700;'><b>🔭 전망:</b> {d['전망']}</p></div>""", unsafe_allow_html=True)

# [계좌별 상세 분석 탭 렌더링 함수: 단일 종목 일원화 적용]
    ...
    with tab_obj:
        ...
        with g_col1:
            if not history_df.empty and history_col in history_df.columns:
                ...
                # 🎯 [v28.3 보정 핵심] 단일 종목 계좌일 때는 점선을 그리지 않고 일원화
                available_stocks = sub_df['종목명'].unique().tolist()
                if len(available_stocks) > 1:
                    # 다종목 계좌일 때만 종목 대조 선택상자와 빨간 점선 표출
                ...
                # 🎯 보정: len == 1 인 경우는 추가 trace 없이 계좌 수익률로 일원화 표출됨
        ...
        with ar_l:
            st.markdown(f"<div class='report-box' style='background-color: rgba(135,206,235,0.05); border: 1px solid rgba(135,206,235,0.2);'><h4 style='color: #87CEEB;'>📋 계좌 내 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>베스트 종목:</b> {b_s['종목명']} ({b_s['전일대비(%)']:+.2f}%)</li><li><b>성과 요약:</b> 현재 누적 손익 <b>{acc_profit:,.0f}원</b>을 기록 중입니다.</li></ul></div>""", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05); border: 1px solid rgba(255,75,75,0.2);'><h4 style='color: #FF4B4B;'>🌍 보유 종목 섹터 분석</h4><ul style='font-size: 0.95em;'><li><b>섹터 동향:</b> {b_s['종목명']}의 탄력적인 움직임이 전체 계좌 수익률을 견인하고 있습니다.</li></ul></div>""", unsafe_allow_html=True)

render_account_tab("서은투자", tabs[1], "서은수익률")
render_account_tab("서희투자", tabs[2], "서희수익률")
render_account_tab("큰스님투자", tabs[3], "큰스님수익률")

st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v28.3 단일 종목 그래프 일원화 완료")
