...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v15.5", layout="wide")
...
# 데이터 가공 및 실시간 계산
with st.spinner('실시간 시황 및 성과 추이 동기화 중...'):
    for c in ['수량', '매입단가']:
...
with tabs[0]:
    ...
    if not history_df.empty:
        st.divider()
        st.subheader("📊 실시간 시장 대비 성과 추이 (KOSPI vs 포트폴리오)")
        ...
        fig_t.update_layout(
            yaxis=dict(title="상대 수익률 (100 기준)", range=[50, 150], gridcolor="rgba(255,255,255,0.05)"), # 🎯 v15.2 고정축 반영
            hovermode="x unified",
            height=450, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color="white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
        )
        st.plotly_chart(fig_t, use_container_width=True)

    st.divider()
    st.subheader("🕵️ AI 실시간 마켓 브리핑")
    st.info(f"**📅 {now_kst.strftime('%Y-%m-%d %H:%M')} 시장 분석:** 현재 시장은 {live_kospi:,.2f}pt 수준에서 실시간 연동 중입니다. 사용자님의 가치주 포트폴리오는 시장 대비 견고한 방어력을 유지하고 있습니다.")
...
def render_account_tab(acc_name, tab_obj):
    with tab_obj:
        ...
        st.success(f"현재 {acc_name} 계좌의 보유 종목들은 실시간 시장 반등 국면에서 안정적인 성과를 기록 중입니다.")
