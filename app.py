...
# 1. 설정 및 스타일 (v30.9 레이아웃 사수)
st.set_page_config(page_title="가족 자산 성장 관제탑 v32.6", layout="wide")
...
def get_advanced_intelligence(name):
    ...
    try:
        res = {"type": "STOCK", "desc": "기업 정보 분석 중...", "div": "N/A", "tp": "N/A", "per": "N/A", "pbr": "N/A", "mc": "N/A", "equity": "N/A", "eps": "N/A", "roe": "N/A", "holdings": ""}
        ...
        if is_etf:
            ...
            for tr in soup.select(".aside tr"):
                if "분배율" in tr.text: res["div"] = tr.find("em").text + "%"
...
if not full_df.empty:
    ...
    # 현재가 파싱 생략 (코드 간소화)
    full_df['현재가'] = full_df['매입단가'] # 임시 (실제 연동 시 파싱 함수 사용)
...
if not history_df.empty:
    ...
    history_df = history_df.dropna(subset=['Date']) # 변환 실패 행 삭제
...
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v32.6</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if not history_df.empty:
        ...
        fig.update_layout(title="📈 자산 수익률 추이", xaxis=dict(type='category'), paper_bgcolor='rgba(0,0,0,0)', font_color="white")
    ...
    r1, r2 = st.columns(2)
    with r1: st.markdown("<div class='report-box'><h4 style='color:#87CEEB;'>🇰🇷 국내 시장 분석</h4><p>2026년 3월 7일 현재, KOSPI 5,000선 안착을 위한 주도주 중심의 강세가 이어지고 있습니다.</p></div>", unsafe_allow_html=True)
    with r2: st.markdown("<div class='report-box'><h4 style='color:#FF4B4B;'>🌍 글로벌 매크로 분석</h4><p>나스닥 AI 랠리와 고환율 환경이 수출 대형주에 우호적으로 작용하고 있습니다.</p></div>", unsafe_allow_html=True)
            ...
            if intel:
                ...
                <div class='insight-card'>
                    ...
                    <div class='insight-grid'>
                        <div><span class='insight-label'>{"분배금 수익률" if intel['type']=="ETF" else "예상 배당률"}</span><br><span class='insight-value'>{intel['div']}</span></div>
                        ...
                        {"<div><span class='insight-label'>주요 구성종목</span><br><span class='insight-value'>" + intel['holdings'] + "</span></div>" if intel['type']=="ETF" else ""}
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v32.6 인텔리전스 가디언")
