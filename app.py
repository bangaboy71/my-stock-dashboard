...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v23.0", layout="wide")
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        # 3. [신규] 그래프 레이아웃 (좌: 수익률 추이, 우: 종목 비중)
        ...
        with col_chart1:
            if not history_df.empty and history_col in history_df.columns:
                fig_trend = go.Figure()
                # 계좌별 누적 수익률 추이
                fig_trend.add_trace(go.Scatter(x=history_df['Date'], y=history_df[history_col], name=f"{acc_name} 수익률", line=dict(color='#87CEEB', width=3), fill='tozeroy'))
                fig_trend.update_layout(title=f"📈 {acc_name} 수익률 변동 추이", height=350, margin=dict(l=20, r=20, t=50, b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white")
                st.plotly_chart(fig_trend, use_container_width=True)
            else:
                st.info("수익률 기록이 충분하지 않습니다. 사이드바의 '저장' 버튼을 눌러 기록을 시작하세요.")
                
        with col_chart2:
            if not sub_df.empty:
                fig_pie = go.Figure(data=[go.Pie(labels=sub_df['종목명'], values=sub_df['평가금액'], hole=.3, textinfo='percent+label')])
                fig_pie.update_layout(title=f"💰 종목별 자산 비중", height=350, margin=dict(l=20, r=20, t=50, b=20), paper_bgcolor='rgba(0,0,0,0)', font_color="white", showlegend=False)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v23.0 계좌 성과 분석 강화")
