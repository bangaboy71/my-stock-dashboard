...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v27.8", layout="wide")

# --- [CSS: 메트릭 폰트 및 리포트 박스 스타일] ---
st.markdown("""
    <style>
    [data-testid="stMetricValue"] { font-size: 1.6rem !important; }
    [data-testid="stMetricLabel"] { font-size: 0.9rem !important; }
    .report-box { padding: 15px; border-radius: 10px; height: 320px; overflow-y: auto; }
    </style>
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v27.8</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with rep_l:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(135,206,235,0.05); border: 1px solid rgba(135,206,235,0.2);'><h4 style='color: #87CEEB;'>📋 포트폴리오 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>시장 대비 성과:</b> KOSPI 대비 <b>{total_daily_rate - float(m_info['KOSPI']['rate'].replace('%','')) :+.2f}%p</b> {'초과 수익' if total_daily_rate > float(m_info['KOSPI']['rate'].replace('%','')) else '하회'} 중</li><li><b>상승 주도:</b> {best_s['종목명']} ({best_s['전일대비(%)']:+.2f}%)</li><li><b>하락 리스크:</b> {worst_s['종목명']} ({worst_s['전일대비(%)']:+.2f}%)</li><li><b>특이사항:</b> 현재 자산은 시장 변동성 대비 방어력이 높은 에너지 및 고배당 섹터 위주로 안정적인 흐름을 보이고 있습니다.</li></ul></div>""", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05); border: 1px solid rgba(255,75,75,0.2);'><h4 style='color: #FF4B4B;'>🌍 시장 동향 및 수급 리포트</h4><ul style='font-size: 0.95em;'><li><b>수급 동향:</b> 외인/기관의 시총 상위주 매도세 속에 특정 실적 개선 기대주(전력기기 등)로 수급 집중 양상.</li><li><b>주도 섹터:</b> 에너지 가격 변동 및 지정학적 이슈로 원자재 관련 섹터가 시장을 견인 중.</li><li><b>전략:</b> 단기 지수 등락에 일희일비하기보다 보유 종목의 펀더멘털과 배당 매력에 집중하는 전략 유지 권고.</li></ul></div>""", unsafe_allow_html=True)
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with ar_l:
            st.markdown(f"""<div class='report-box' style='background-color: rgba(135,206,235,0.05); border: 1px solid rgba(135,206,235,0.2);'><h4 style='color: #87CEEB;'>📋 계좌 내 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>베스트 종목:</b> {b_s['종목명']} ({b_s['전일대비(%)']:+.2f}%)</li><li><b>워스트 종목:</b> {w_s['종목명']} ({w_s['전일대비(%)']:+.2f}%)</li><li><b>성과 요약:</b> 지수 대비 변동폭이 낮아 안정적인 수익 구조를 유지하고 있습니다. 누적 손익 <b>{acc_profit:,.0f}원</b> 기록 중.</li></ul></div>""", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05); border: 1px solid rgba(255,75,75,0.2);'><h4 style='color: #FF4B4B;'>🌍 보유 종목 섹터 분석</h4><ul style='font-size: 0.95em;'><li><b>주요 섹터 동향:</b> 현재 계좌의 주력 섹터인 {b_s['종목명']} 관련 업황이 시장 평균 대비 탄력적인 수급 흐름을 보이고 있습니다.</li><li><b>특이사항:</b> 개별 종목 재료에 의한 상승 여력이 충분하며, 전일 대비 안정적인 우상향 패턴을 형성 중입니다.</li></ul></div>""", unsafe_allow_html=True)

render_account_tab("서은투자", tabs[1], "서은수익률")
render_account_tab("서희투자", tabs[2], "서희수익률")
render_account_tab("큰스님투자", tabs[3], "큰스님수익률")

st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v27.8 인텔리전스 리포트 완전 복구")
