...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v25.1", layout="wide")
...
full_df['전일대비(%)'] = ((full_df['현재가'] / full_df['전일종가'] - 1) * 100).fillna(0) # 🎯 신규 삽입 컬럼
...
if st.sidebar.button("🔄 실시간 시세 갱신"): st.cache_data.clear(); st.rerun()
st.sidebar.divider()
if st.sidebar.button("💾 오늘의 결과 저장하기"): record_performance()

# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v25.1</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    # 🎯 총괄표에도 전일대비 변동(가중평균) 삽입 가능하나, 요청하신 '각 표' 형태를 유지하기 위해 계좌별 요약 표 출력
    ...
    with tab_obj:
        ...
        with col_c1:
            ...
            if not history_df.empty and history_col in history_df.columns:
                ...
                fig.update_layout(title=f"📈 {acc_name} 성과분석 (vs KOSPI)", xaxis=dict(tickformat="%Y-%m-%d"), yaxis=dict(ticksuffix="%"), height=400, paper_bgcolor='rgba(0,0,0,0)', font_color="white")
        ...
        st.success(f"🔍 AI 진단: {acc_name} 계좌는 {sub_df.sort_values('평가금액', ascending=False).iloc[0]['종목명']} 중심 포트폴리오입니다.")

render_account_tab("서은투자", tabs[1], "서은수익률")
render_account_tab("서희투자", tabs[2], "서희수익률")
render_account_tab("큰스님투자", tabs[3], "큰스님수익률")

st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v25.1 전일 대비 변동률 지표 삽입 완료")
