...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v17.1", layout="wide")

# --- [GID 및 기준 지수 설정] ---
STOCKS_GID = "301897027"
TREND_GID = "1055700982"
BASE_KOSPI = 5510.87  # 🎯 세로축 100의 기준이 되는 고정 KOSPI 지수
...
# 데이터 가공
with st.spinner('정밀 시세 분석 및 차트 렌더링 중...'):
    for c in ['수량', '매입단가']:
...
with tabs[0]:
    ...
    if not history_df.empty:
        ...
        # 1. KOSPI 지수 (사용자 지정 기준치 5510.87 = 100 적용)
        fig_t.add_trace(go.Scatter(x=display_df['Date'], y=(display_df['KOSPI'] / BASE_KOSPI) * 100, 
                                   name=f'KOSPI (100={BASE_KOSPI:,.2f})', line=dict(dash='dash', color='gray')))
        
        # 2. 계좌별 수익률 곡선 (각 계좌의 수익률을 기준 시점 대비 정규화하여 표기)
        acc_c = {'서은수익률': '#FF4B4B', '서희수익률': '#87CEEB', '큰스님수익률': '#00FF00'}
        for c, clr in acc_c.items():
            if c in display_df.columns:
                # 수익률 1% 상승을 지수 1pt 상승과 동일한 가중치로 표현
                nv = 100 + display_df[c] - ( (history_df['KOSPI'].iloc[0] / BASE_KOSPI * 100) - 100 ) # 기준점 보정 로직
        ...
        # 🎯 [요청 반영] 가로축 시간 구분선 (09:00, 15:30)
        # 오늘 날짜를 기준으로 구분선 추가
        today_str = now_kst.strftime('%Y-%m-%d')
        fig_t.add_vline(x=f"{today_str} 09:00", line_width=1, line_dash="solid", line_color="rgba(255,255,255,0.2)")
        fig_t.add_vline(x=f"{today_str} 15:30", line_width=1, line_dash="solid", line_color="rgba(255,255,255,0.2)")
        
        fig_t.add_annotation(x=f"{today_str} 09:00", y=145, text="장 시작", showarrow=False, font=dict(color="gray", size=10))
        fig_t.add_annotation(x=f"{today_str} 15:30", y=145, text="장 마감", showarrow=False, font=dict(color="gray", size=10))

        # 레이아웃 설정
        fig_t.update_layout(
            yaxis=dict(title=f"상대 성과 (100 = KOSPI {BASE_KOSPI:,.2f})", range=[50, 150], gridcolor="rgba(255,255,255,0.05)"),
            hovermode="x unified",
            height=480, 
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | 기준 지수: {BASE_KOSPI:,.2f}")
