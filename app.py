...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v29.3", layout="wide")
...
def load_data_with_retry(worksheet_name, ttl_val):
    try:
        ...
        if "503" in str(e) or "UNAVAILABLE" in str(e):
            st.warning("📡 구글 시트 연결이 일시적으로 원활하지 않습니다. 5초 후 자동으로 재시도합니다.")
            time.sleep(5)
            st.rerun()
...
def get_market_status():
    ...
    try:
        ...
        market["NASDAQ"] = {"rate": "+0.85%"} # 참고용 텍스트
    except: pass
    return market
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v29.3</h1>", unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v29.3 완전 복구 및 NameError 해결 버전")
