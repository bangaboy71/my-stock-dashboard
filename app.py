...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v27.2", layout="wide")
...
def get_market_status():
    ...
    except: market["KOSPI"] = {"now": "0", "diff": "0", "rate": "0%"}
    return market
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v27.2</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    best_acc = sum_acc.sort_values('전일대비(%)', ascending=False).iloc[0]['계좌명']
    best_stock = full_df.sort_values('전일대비(%)', ascending=False).iloc[0]
    
    r_c1, r_c2 = st.columns(2)
    with r_c1:
        st.info(f"**Alpha 분석:** 시장 대비 **{alpha:+.2f}%p** 초과 성과 기록 중")
    with r_c2:
        st.info(f"**베스트:** 계좌(**{best_acc}**) / 종목(**{best_stock['종목명']}**)")
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        if sub_df.empty:
            st.warning(f"{acc_name}에 대한 데이터를 찾을 수 없습니다.")
            return
        ...
        with col_c1:
            ...
            if not history_df.empty and history_col in history_df.columns:
                ...
                bk_kospi = history_df['KOSPI'].iloc[0] if history_df['KOSPI'].iloc[0] != 0 else 1
                kospi_yield = ((history_df['KOSPI'] / bk_kospi) - 1) * 100
                fig.add_trace(go.Scatter(x=history_dates, y=kospi_yield, name='KOSPI 지수', line=dict(dash='dash', color='gray')))
                
                if len(available_stocks) > 1:
                    selected_stock = st.selectbox(f"📍 {acc_name} 대조 종목 선택", available_stocks, key=f"sel_{acc_name}")
                    short_acc = acc_name.replace("투자", ""); target_stock_name = selected_stock.replace(' ', '')
                ...
                fig.update_layout(xaxis=dict(type='category'), yaxis=dict(ticksuffix="%"), height=400, paper_bgcolor='rgba(0,0,0,0)', font_color="white")
        ...
        st.success(f"🔍 **AI 분석:** {acc_name} 계좌는 {sub_df.sort_values('평가금액', ascending=False).iloc[0]['종목명']} 비중이 가장 높습니다.")
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v27.2 레이아웃 복구 완료")
