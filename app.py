...
DIVIDEND_SCHEDULE = {
    ...
    "현대글로비스": [4, 8],        # 반기 배당
    "테스": [4], "에스티팜": [4], "일진전기": [4],  # 결산 배당
    "KODEX200타겟위클리커버드콜": list(range(1, 13))  # 월배당
}
        ...
        with st.container(border=True):
            if not history_df.empty:
                ...
                fig_total.update_layout(
                    ...
                    height=380, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(255,255,255,0.02)',
                    font_color="white", legend=dict(orientation="h", yanchor="bottom", y=-0.4, xanchor="center", x=0.5),
    ...
    with chart_col2:
        with st.container(border=True):
            ...
            fig_cal.update_layout(
                ...
                height=380, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(255,255,255,0.02)',
                font_color="white", margin=dict(t=80, b=40, l=20, r=20)
            )
    ...
    with tab_obj:
        ...
        with g_left:
            with st.container(border=True):
                ...
                fig_a_cal.update_layout(title=dict(text="📅 월별 배당 예측", x=0.02), height=300,
                    paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(255,255,255,0.02)',
                    font_color="white", margin=dict(t=80, b=20))
        ...
        with g_right:
            with st.container(border=True):
                ...
                fig_bar.update_layout(title=dict(text="📊 자산 비중", x=0.02), height=300,
                    paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(255,255,255,0.02)',
                    font_color="white", margin=dict(t=80, b=20))
        ...
        with st.container(border=True):
            if not history_df.empty:
                ...
                fig_acc.update_layout(
                    title=dict(text=f"📈 {sel} 성과 분석 추이", x=0.02), height=400,
                    paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(255,255,255,0.02)',
                    font_color="white", legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5),
                    margin=dict(t=80, b=80), xaxis=dict(type='category', tickangle=-45)
                )
