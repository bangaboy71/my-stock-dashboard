...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v30.6", layout="wide")

# --- [CSS: v30.2 마스터 스타일 완벽 복구] ---
st.markdown("""
    <style>
...
def record_performance():
    ...
    new_row = {"Date": today, "KOSPI": float(kospi_now), "서은수익률": acc_sum.get('서은투자', 0), "서희수익률": acc_sum.get('서희투자', 0), "큰스님수익률": acc_sum.get('큰스님투자', 0)}
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v30.6</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    # 🕵️ AI 데일리 심층 리포트 (v30.2 레이아웃 + 2026 팩트 동기화)
    st.divider()
    ...
    with rep_l:
        ...
        st.markdown(f"""<div class='report-box'>
            ...
            <p>2026년 3월 7일 현재, 국내 증시는 <b>KOSPI {m_idx['KOSPI']['now']}선</b>을 돌파하며 역사적 초강세장(Bull Market) 국면에 진입했습니다. 고환율 리스크에도 불구하고 글로벌 AI 반도체 공급망 내 독보적 지위가 부각되며 외국인과 기관의 강력한 수급 유입이 지수를 견인하고 있습니다.</p>
            <div class='market-tag'>KOSPI 시장 분석</div>
            <p>삼성전자와 SK하이닉스 등 시총 상위주가 지수의 전고점을 지속적으로 경신하고 있습니다. 특히 5,000포인트 시대가 열리며 과거의 박스권 한계를 완전히 불식시켰으며, 밸류업 프로그램의 정착으로 인한 저PBR 대형주들의 재평가가 지수의 하단을 견고하게 지지하고 있습니다.</p>
            <div class='market-tag'>KOSDAQ 시장 분석</div>
            <p>코스닥 역시 1,000선을 상회하며 제약/바이오 대장주들의 임상 성공 및 기술 수출 성과가 지수 성장을 주도하고 있습니다. 2차전지 섹터는 미국 행정부의 정책 변수 속에서도 리튬가 안정에 따른 실적 턴어라운드를 모색 중이며, 로봇/AI 섹터로의 유동성 유입이 뚜렷합니다.</p>
    ...
    with rep_r:
        st.markdown(f"""<div class='report-box'>
            ...
            <p>나스닥은 기술주 중심의 강력한 랠리 속에 AI 실질 실적 검증 단계에 진입했습니다. 어제 밤 일부 차익 실현 매물이 출회되었으나, 빅테크 기업들의 강력한 현금 흐름이 글로벌 위험자산 선호 심리를 강력하게 지지하고 있습니다.</p>
            <div class='market-tag'>원/달러 환율 및 매크로 현황</div>
            <p><b>환율 사실관계:</b> 현재 환율은 <b>1,450~1,480원선</b>의 초고환율 국면에 있습니다. 미 행정부의 통상 압박과 2026년 11월 예정된 미국 중간선거 변수가 맞물려 환율 변동성이 극대화되고 있으나, 우리 수출 대형주들에게는 장부상 이익 확대의 기회로 작용하고 있습니다.</p>
            <div class='market-tag'>원자재 및 리스크 관리</div>
            <p>금 현물은 지정학적 불안 지속으로 사상 최고가 부근에서 강력한 지지선을 형성 중입니다. 안전자산과 초우량 기술주로의 자금 양극화 현상이 뚜렷하므로 리스크 관리에 만전을 기해야 합니다.</p>
        </div>""", unsafe_allow_html=True)

    # 📊 관심 섹터 인텔리전스 (v30.2의 6개 섹터 완벽 복구)
    st.divider()
    st.subheader("📊 관심 섹터별 인텔리전스 (주도주 및 심층 분석)")
    # 
    sec_cols = st.columns(3)
    sectors = {
        "반도체 / IT": {"주도주": "삼성전자, SK하이닉스, 한미반도체, 이수페타시스, DB하이텍", "시황": "글로벌 AI 칩 수요 폭증 및 HBM3E 공급 부족 지속.", "수급": "외인/기관 동반 순매수세가 지수 성장을 주도.", "뉴스": "차세대 패키징 공정 수율 개선 및 대규모 수주 공시.", "전망": "AI 서버 투자 사이클 지속으로 하반기 실적 퀀텀 점프."},
        "전력 / ESS": {"주도주": "LS ELECTRIC, 일진전기, 현대일렉트릭, 효성중공업", "시황": "미국 내 노후 전력망 교체 및 AI 데이터센터 전력난 수혜.", "수급": "수주 잔고 기반 실적주로서 외인 매집 지속.", "뉴스": "북미 변압기 수출 잔고 최고치 갱신 및 공장 증설 발표.", "전망": "향후 3년 이상의 장기 슈퍼 사이클 진입 가시화."},
        "배터리 / 에너지": {"주도주": "LG에너지솔루션, 에코프로비엠, POSCO홀딩스", "시황": "전기차 캐즘 국면에도 불구 전고체 기술 상용화 기대감.", "수급": "개인 매수세 결집 국면이나 외인 숏커버링 물량 포착.", "뉴스": "차세대 폼팩터 배터리 샘플 공급 및 합작법인 설립.", "전망": "정책 변동성 통과 후 기술력 보유 기업 중심 재편."},
        "바이오 / 헬스케어": {"주도주": "삼성바이오로직스, 알테오젠, HLB, 리가켐바이오", "시황": "고금리 환경에도 개별 임상 결과에 따른 강력한 랠리.", "수급": "기술 수출 기대감에 따른 스마트 머니 유입 지속.", "뉴스": "글로벌 빅파마와의 대규모 라이선스 아웃 및 FDA 승인.", "전망": "실질적 기술료 유입에 따른 시가총액 재평가 가중."},
        "모빌리티 / 로봇": {"주도주": "현대차, 기아, 두산로보틱스, 레인보우로보틱스", "시황": "휴머노이드 상용화 및 고환율에 따른 수출 경쟁력 강화.", "수급": "기관 장기 포트폴리오 내 비중 확대 중.", "뉴스": "지능형 로봇법 개정안 통과 및 M&A 기대감 유입.", "전망": "스마트 팩토리 수요 증대로 인한 로봇 시장 본격 개화."},
        "소비재 / 뷰티": {"주도주": "아모레퍼시픽, 브이티, 코스메카코리아", "시황": "K-뷰티의 북미 아마존 내 점유율 및 매출 폭증.", "수급": "실적 서프라이즈 기반의 외인 지속적 롱 포지션.", "뉴스": "해외 유통 채널 다변화 및 사상 최대 해외 매출 달성.", "전망": "브랜드 파워 강화에 따른 안정적 영업이익률 확보."}
    }
...
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with ar_l:
            st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color: #87CEEB;'>📋 계좌 총평</h4><p>현재 {acc_name} 계좌는 {b_s['종목명']}의 주도력에 힘입어 시장 평균 대비 안정적인 흐름을 유지하고 있습니다. 초강세 지수 환경 속에서 보유 종목의 핵심 기술력과 실적 모멘텀이 변함없이 견조함을 확인 중입니다.</p></div>", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"<div class='report-box' style='height:250px;'><h4 style='color: #FF4B4B;'>🌍 업황 대응 전략</h4><p>1,450원선 이상의 환율 변동성이 실적에 미치는 긍정적 효과를 극대화하기 위해 수출 대형주 중심의 비중을 유지하며, 미 중간선거 변수에 따른 정책 리스크를 실시간 모니터링 중입니다.</p></div>", unsafe_allow_html=True)

render_account_tab("서은투자", tabs[1], "서은수익률")
render_account_tab("서희투자", tabs[2], "서희수익률")
render_account_tab("큰스님투자", tabs[3], "큰스님수익률")

st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v30.6 마스터피스 리스토어 버전")
