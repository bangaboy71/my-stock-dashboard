...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 통합 자산 관제탑 v13.5", layout="wide")

# --- [수정 필수] 사용자님의 실제 시트 GID로 교체 ---
STOCKS_GID = "301897027"  # 종목 탭 GID
TREND_GID = "확인하신_트렌드_GID"  # 트렌드 데이터 탭 GID
...
# 전역 데이터 전처리 (모든 종목 시세 업데이트)
with st.spinner('전체 가족 자산을 분석 중입니다...'):
    for c in ['수량', '매입단가']:
...
with tab_summary:
    ...
    # 퇴직 D-Day (2026-03-31 가정)
    d_day = (datetime(2026, 3, 31) - datetime.now()).days
    
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("가족 총 평가액", f"{total_eval:,.0f}원", f"{total_profit:+,.0f}원")
    c2.metric("총 투자 원금", f"{total_buy:,.0f}원")
    c3.metric("통합 누적 수익률", f"{total_roi:.2f}%", f"{total_roi:+.2f}%")
    c4.metric("퇴직까지 남은 날", f"D-{d_day}일" if d_day > 0 else "축 퇴직!")

    # 2. 시장 대비 성과 추이 (전 계좌 통합 차트)
    if not history_df.empty:
        st.divider()
        st.subheader("📊 시장 대비 성과 추이 (KOSPI vs 전 계좌 통합)")
        ...
        fig_t.update_layout(height=500, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color="white",
...
# 하단 시장 평론 (동일 유지)
st.divider()
st.subheader("🕵️ 보유종목 기반 시장 평론")
st.info("글로벌 변동성 장세 속에서도 사용자님의 현대차2우B, KT&G 등 배당주 중심 포트폴리오는 견고한 방어력을 보여주고 있습니다. 자녀분들의 장기 적립식 투자 역시 시간의 힘을 믿고 나아갈 시점입니다.")
st.caption(f"최종 업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 은퇴가 가까워질수록 자산은 더 단단해집니다.")
