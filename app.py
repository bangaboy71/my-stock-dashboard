...
# 1. 설정 및 연결
st.set_page_config(page_title="가족 자산 성장 관제탑 v28.1", layout="wide")
...
# --- UI 메인 ---
st.markdown(f"<h1 style='text-align: center; color: #87CEEB;'>🌐 AI 금융 통합 관제탑 v28.1</h1>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    # 🕵️ AI 관제탑 데일리 분석 리포트 (v27.8/v28.0 사양 유지)
    st.divider()
    ...
    with rep_l:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(135,206,235,0.05); border: 1px solid rgba(135,206,235,0.2);'><h4 style='color: #87CEEB;'>📋 포트폴리오 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>시장 대비 성과:</b> KOSPI 대비 <b>{total_daily_rate - float(m_info['KOSPI']['rate'].replace('%','')) :+.2f}%p</b> {'초과 수익' if total_daily_rate > float(m_info['KOSPI']['rate'].replace('%','')) else '하회'} 중</li><li><b>전략 가이드:</b> 섹터 순환매 장세에서 포트폴리오의 방어적 성격이 수익률을 안정적으로 지지하고 있습니다.</li></ul></div>""", unsafe_allow_html=True)
    with rep_r:
        st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05); border: 1px solid rgba(255,75,75,0.2);'><h4 style='color: #FF4B4B;'>🌍 시장 동향 및 수급 리포트</h4><ul style='font-size: 0.95em;'><li><b>수급 현황:</b> 대형주 위주의 기관 매도세가 있으나, 중소형 테마주와 특정 에너지 섹터로의 수급 쏠림 현상 포착.</li></ul></div>""", unsafe_allow_html=True)

    # 📊 관심 섹터 동향 분석 (6대 섹터 유지)
    st.divider()
    st.subheader("📊 관심 섹터 동향 분석")
    sec_cols = st.columns(3)
    sectors = {
        "반도체": {"trend": "HBM 수요 견조 및 공급망 다변화", "leader": "삼성전자, SK하이닉스", "news": "AI 반도체 시장 확대에 따른 후공정 장비주 강세"},
        "로봇/AI": {"trend": "지능형 로봇법 개정 및 서비스 로봇 확산", "leader": "두산로보틱스, 레인보우로보틱스", "news": "대기업의 AI 로봇 스타트업 인수 기대감 유입"},
        "의약/화장품": {"trend": "K-뷰티 북미 매출 폭증 지속", "leader": "삼성바이오로직스, 아모레퍼시픽", "news": "실적 기대감에 따른 화장품 중소형주 강세 흐름"},
        "ESS/전력": {"trend": "북미 전력망 교체 및 데이터센터 전력 수요", "leader": "LS ELECTRIC, 일진전기", "news": "슈퍼 사이클 진입에 따른 전력 설비주 신고가 릴레이"},
        "방산/우주": {"trend": "수출국 다변화 및 차세대 발사체 개발", "leader": "한화에어로스페이스, LIG넥스원", "news": "K-방산의 글로벌 점유율 확대 및 추가 수주 소식"},
        "배터리/전고체": {"trend": "차세대 배터리 로드맵 가시화 및 저가 매수", "leader": "LG에너지솔루션, 이수스페셜티케미컬", "news": "전고체 상용화 기대감에 따른 관련 소재주 반등"}
    }
    for i, (name, data) in enumerate(sectors.items()):
        with sec_cols[i % 3]:
            st.markdown(f"""<div class='sector-box'><div class='sector-title'>{name}</div><p style='font-size: 0.85em;'><b>🌡️ 시황:</b> {data['trend']}</p><p style='font-size: 0.85em;'><b>👑 주도:</b> {data['leader']}</p><p style='font-size: 0.85em;'><b>📰 뉴스:</b> {data['news']}</p></div>""", unsafe_allow_html=True)

# [계좌별 상세 분석 탭 렌더링 함수 - 복구 완료]
def render_account_tab(acc_name, tab_obj, history_col):
    with tab_obj:
        ...
        with g_col1:
            if not history_df.empty and history_col in history_df.columns:
                ...
                if len(available_stocks) > 1:
                    ...
                    # history_df에서 '계좌_종목수익률' 형태의 컬럼 탐색
                    ...
                    # 종목이 하나인 경우(예: 큰스님) 자동으로 해당 종목 추이 탐색
                    short_acc = acc_name.replace("투자", "")
        ...
        with ar_l:
            st.markdown(f"""<div class='report-box' style='background-color: rgba(135,206,235,0.05); border: 1px solid rgba(135,206,235,0.2);'><h4 style='color: #87CEEB;'>📋 계좌 내 성과 분석</h4><ul style='font-size: 0.95em;'><li><b>베스트 종목:</b> {b_s['종목명']} ({b_s['전일대비(%)']:+.2f}%)</li><li><b>워스트 종목:</b> {w_s['종목명']} ({w_s['전일대비(%)']:+.2f}%)</li><li><b>성과 요약:</b> 현재 누적 손익 <b>{acc_profit:,.0f}원</b>으로 안정적으로 운용 중입니다.</li></ul></div>""", unsafe_allow_html=True)
        with ar_r:
            st.markdown(f"""<div class='report-box' style='background-color: rgba(255,75,75,0.05); border: 1px solid rgba(255,75,75,0.2);'><h4 style='color: #FF4B4B;'>🌍 보유 종목 섹터 분석</h4><ul style='font-size: 0.95em;'><li><b>섹터 동향:</b> {b_s['종목명']} 관련 업황의 탄력성이 계좌 전체의 수익 지지력을 강화하고 있습니다.</li></ul></div>""", unsafe_allow_html=True)
...
st.caption(f"최종 업데이트: {now_kst.strftime('%Y-%m-%d %H:%M:%S')} (KST) | v28.1 종목별 추이 그래프 및 대조 기능 완전 복구")
