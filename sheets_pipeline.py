...
def save_trend(
    ...
    try:
        ...
        account_list = [
            {"name": acc, "yield_col": f"{acc.replace('투자','')}수익률"}
...
def save_market_log(
    ...
    try:
        ...
        # 최근 90일치만 보존 (시트 용량 관리)
        if "타임스탬프" in combined.columns:
            combined["_dt"] = pd.to_datetime(combined["타임스탬프"], errors="coerce")
            cutoff = now_kst.replace(tzinfo=None) - timedelta(days=90)
            combined = combined[combined["_dt"].dt.tz_localize(None) >= cutoff]
...
def save_collection_log(
    ...
    try:
        ...
        if "타임스탬프" in combined.columns:
            combined["_dt"] = pd.to_datetime(combined["타임스탬프"], errors="coerce")
            cutoff = now_kst.replace(tzinfo=None) - timedelta(days=180)
            combined = combined[combined["_dt"].dt.tz_localize(None) >= cutoff]
            combined = combined.drop(columns=["_dt"])
